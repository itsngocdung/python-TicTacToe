...
# Khởi tạo hệ thống âm thanh của pygame
pygame.init()
pygame.mixer.init()
...
def next_turn(row, column):
    ...
    if buttons[row][column]['text'] == "" and check_winner() is False:
        ...
        if player == players[0]:
            # buttons[row][column].config(fg="black")
...
def new_game():
    ...
    if dark_mode is False:
        for row in range(10):
            for column in range(10):
                buttons[row][column].config(text="", bg="#F0F0F0")#white
    else:
        for row in range(10):
            for column in range(10):
                buttons[row][column].config(text="", bg="#333333")#dark
...
label = Label(text= "Lượt của " + player, font = ("consolas", 40), fg="red")
label.pack(side="top")
...
for row in range(10):
    for column in range(10):
        buttons[row][column] = Button(frame, text="", font=("consolas", 20), width=3, height=1,
